...
export function Navbar({ session }: NavbarProps) {
  ...
  return (
    ...
    >
      <div className="glass-premium rounded-full p-2 flex items-center gap-6 border-white/10 bg-black/60 backdrop-blur-3xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.7)] pointer-events-auto">
        {/* Logo Section */}
        ...
        <div className="flex items-center gap-2">
          {navLinks.map((link) => {
            ...
            return (
              <Link 
                key={link.label}
                href={link.href} 
                className={`text-label-caps !text-[12px] px-5 py-2.5 rounded-full transition-all relative group ${isActive ? '!text-white bg-white/10' : '!text-white/40 hover:!text-white hover:bg-white/5'}`}
              >
                {link.label}
                {isActive && (
                  <motion.div 
          ...
          {session?.user ? (
            <Link 
              href="/dashboard"
              className="flex items-center gap-3 px-4 py-1.5 rounded-full bg-white/5 hover:bg-white/10 transition-colors group border border-white/5 ml-2"
            >
              <div className="flex flex-col items-end">
                <span className="text-label-caps !text-white !text-[8px] !tracking-widest">
                  {session.user.name?.split(' ')[0] || 'OPERATOR'}
                </span>
                <span className="text-micro !text-primary !text-[6px] tracking-[0.2em]">ACTIVE</span>
              </div>
              {session.user.image ? (
            ...
            <Link href="/login" className="btn-vibrant !py-2.5 !px-8 !rounded-full !shadow-none !border-none !opacity-100 flex items-center gap-2 ml-2">
              <span className="!text-white !opacity-100 font-display font-bold text-[11px] normal-case tracking-tight">Portal Access</span>
            </Link>
          )}
        </div>
      </div>
